...
class DetallesPerfil(TestCase):
    def setUp(self):
        ...
        userMaria.save()
        piso_maria = Piso.objects.create(zona="Calle Marqués Luca de Tena 3", descripcion="Descripción de prueba 2")
        maria=Usuario.objects.create(usuario=userMaria, fecha_nacimiento="2000-1-1",lugar="Sevilla", telefono="+34666777222",genero='F',estudios="Informática", sms_validado=True)
        maria.save()

        userPepe = User(username='usuario2')
        userPepe.set_password('qwery')
        userPepe.save()
        pepe= Usuario.objects.create(usuario=userPepe, fecha_nacimiento="2000-1-1",lugar="Sevilla", telefono='+34111222333',genero='F',estudios="Informática", sms_validado=True)
        pepe.save()

        user_no_sms = User(username='noSMS')
        user_no_sms.set_password('qwery')
        user_no_sms.save()
        noSMS = Usuario.objects.create(usuario=user_no_sms, fecha_nacimiento="2000-1-1", lugar="Sevilla", telefono="+34666555444",genero='F',estudios="Informática", sms_validado=False)
        noSMS.save()
    ...
    def test_positive_detalles(self):
        c = Client()
        login = c.login(username="Maria", password="asdfg")
